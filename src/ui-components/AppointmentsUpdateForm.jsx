...
export default function AppointmentsUpdateForm(props) {
  const {
    id: idProp,
    appointments: appointmentsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = appointmentsRecord
      ? { ...initialValues, ...appointmentsRecord }
      : initialValues;
    setAppointment_date(cleanValues.appointment_date);
    setCreated_at(cleanValues.created_at);
    setStart_time(cleanValues.start_time);
    setEnd_time(cleanValues.end_time);
    setStatus(cleanValues.status);
    setAssigned_to(cleanValues.assigned_to);
    setPatientID(cleanValues.patientID);
    setStaffID(cleanValues.staffID);
    setErrors({});
  };
  const [appointmentsRecord, setAppointmentsRecord] = React.useState(
    appointmentsModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Appointments, idProp)
        : appointmentsModelProp;
      setAppointmentsRecord(record);
    };
    queryData();
  }, [idProp, appointmentsModelProp]);
  React.useEffect(resetStateValues, [appointmentsRecord]);
  const validations = {
    appointment_date: [],
    created_at: [],
    start_time: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Appointments.copyOf(appointmentsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AppointmentsUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || appointmentsModelProp)}
